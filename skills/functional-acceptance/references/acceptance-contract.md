...
| ID | Required outcome and source | Actor / object / action or entrypoint | Preconditions / independent expectation | Candidate observation and qualification | Order and reason / gap |
| --- | --- | --- | --- | --- | --- |
| O1 | … | … | … | … | … |
...
Account for every explicit result and constraint. One obligation is one independently verifiable claim: if one action can pass while another violates the promise, give them separate rows. Inspect the relevant entrypoints and assertions, not just test names. For each reused assertion, record why its setup forces the decisive precondition and one false-green execution you ruled out; if that execution remains possible, the assertion is only supporting evidence. Mark a check proposed until observed. Expected results come from requirements or independent fixtures, not captured product output; a digest alone supplies no business expectation.
...
The contract is ready when every requested outcome has checks or gaps, each check has an independent expectation and qualified observation, and the first executable checks target the most consequential unknowns after necessary setup. For a claimed fix, at least one decisive observation must use an arrangement independent from the fix's existing regression arrangement unless the actual user boundary is observed end to end. A shared fixture is a dependency, not a reason to defer an independent check behind unrelated work. Missing execution access need not block a plan; for execution, retain blocked obligations while continuing ready checks within authority.
