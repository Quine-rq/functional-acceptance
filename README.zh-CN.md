...
> 可安装的开发预览版，尚非受支持的正式版本。请先在隔离测试项目中试用；采用 [MIT 许可证](LICENSE) 发布。
...
正式发布前，还需要验证第二种宿主稳定执行完整流程，让外部开发者在自己的项目中试用，并证明相对基线的实际收益。[外部开发者试用方案](evals/HUMAN-PILOT.md)说明了陌生用户如何在不上传项目或原始证据的前提下参与；[本轮原生评估](NATIVE-FOLLOWUP-RESULTS.md)逐项说明已做什么、仍有哪些失败。本地测试通过不等于成熟产品。
...
提交普通问题前请阅读 [SUPPORT.md](SUPPORT.md)；安全问题请先阅读 [SECURITY.md](SECURITY.md)，贡献代码或说明前请阅读 [CONTRIBUTING.md](CONTRIBUTING.md)。
