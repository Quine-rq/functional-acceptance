...
Skill 提供验收方法，实际操作由助手和项目工具执行。它不替代现有测试框架，也不替你批准上线。可以先运行下面的本地导出示例；另有一份 [linkding 实测记录](evals/results/2026-09-08-linkding/README.md)，展示真实浏览器与数据库流程，以及过程中需要的作者协助。
...
[第二项目 linkding 的实测](evals/results/2026-09-08-linkding/README.md)已覆盖真实浏览器登录、服务重启后的书签持久化、跨账号隔离、编辑和定向删除，并核对 SQLite 记录。独立 Skill 会话因本地运行权限受阻；完整流程是在作者协助、修正回归脚本后通过的。记录保留了失败过程和原生重跑说明，不能据此宣称已经能无人干预地跨项目复用，或正式支持浏览器集成。外部服务和手机仍未验证。
...
接下来需要由独立执行者按已审阅的 linkding 回归包复跑，再重复验证行为稳定性、补测另一种宿主的实际调用和缺能力时的表现，并对比同一个 Agent 使用与不使用 Skill 的效果。这些验证先于正式发布。
