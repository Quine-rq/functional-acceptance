...
"""
    identity_result = invoke('00-identity', ['-c', probe], module=False)
    assert identity_result['returncode'] == 0, 'Identity preflight failed; inspect 00-identity.stderr. Business checks are unverified.'
    ...
    files = list((ROOT / 'sqlite_utils').rglob('*.py')) + [ROOT / 'pyproject.toml', ROOT / 'ONBOARDING-REQUIREMENTS.md', ROOT / 'acceptance/__init__.py', ROOT / 'acceptance/test_onboarding.py', ROOT / 'acceptance/run.py', ROOT / 'acceptance/lifecycle.py']
