(() => {
  ...
  document.addEventListener('turbo:frame-load', event => {
    ...
    record('injection.armed', {delay_ms: 150, trigger: 'target turbo:frame-load'});
    window.requestAnimationFrame = function(callback) {
      window.requestAnimationFrame = originalRAF;
      const stack = (new Error().stack || '').split('\n').filter(line => line.includes('http://127.0.0.1:18741/static/bundle.js:'));
      record('injection.intercepted', {delay_ms: 150, raf_restored: window.requestAnimationFrame === originalRAF, registration_stack: stack, callback_source: String(callback)});
      let timer = null;
      let delivered = false;
      const restoreCancel = () => {
      ...
      // Preserve cancellation for the one delayed callback; all other IDs pass through.
