...
def child():
    ...
    try:
        ...
        for group_name in ('modal-visible', 'frame-complete'):
        ...
        summary = {'kind':'read-only-ui-lifecycle-diagnostic','elapsed_ms':round((time.monotonic()-start)*1000),'bookmark_id':14,'groups':groups,'error_type':error_type,'blocked_write_attempts':blocked,'confirm_clicked':False,'baseline_ids':[r['id'] for r in baseline],'final_ids':[r['id'] for r in H['bookmark_rows']()],'all_bookmark_rows_unchanged':H['bookmark_rows']()==baseline,'source_and_environment_unchanged':H['identity']()==identity,'old_final_results_unchanged':H['digest'](ROOT/'final-results.json')==old_hash,'processes':[{'name':n,'pid':p.pid,'returncode':p.poll()} for n,p in processes],'ports_free':H['ports_free']()}
