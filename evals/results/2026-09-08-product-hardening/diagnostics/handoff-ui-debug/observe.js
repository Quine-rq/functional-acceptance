(() => {
  ...
  for (const name of ['turbo:before-cache', 'turbo:frame-render', 'turbo:frame-load']) {
    document.addEventListener(name, event => {
      if (name !== 'turbo:before-cache' && event.target?.id !== 'details-modal') return;
      record(name, {phase: 'dispatch'});
      queueMicrotask(() => record(name, {phase: 'after-dispatch'}));
    }, true);
  }
