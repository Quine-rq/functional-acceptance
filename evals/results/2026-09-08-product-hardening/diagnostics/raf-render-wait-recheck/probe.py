...
def child():
    ...
    turbo_visit_completed = runpy.run_path(str(H['PACK']/'support.py'))['turbo_visit_completed']
    signal.signal(signal.SIGTERM, stopped)
    signal.signal(signal.SIGINT, stopped)
    os.chmod(HERE, 0o700)
    baseline = H['bookmark_rows']()
    identity = H['identity']()
    if H['digest'](ROOT/'bookmarks/static/bundle.js') != 'b3bd8d18e0e05750a2440bf6b33ebdb3769422a134bbc7db5d13c342fb0b85f9':
        raise RuntimeError('Bundle hash does not match the independently measured callsite; no servers started')
    ...
    try:
        ...
        for group_name in ('render-callsite-injection',):
            ...
            for number in range(1,4):
                ...
                try:
                    ...
                    view = page.locator('ul.bookmark-list > li[data-bookmark-id="14"]').get_by_text('View',exact=True)
                    href = view.get_attribute('href')
                    if not href:
                        raise RuntimeError('Details link has no navigation target')
                    with turbo_visit_completed(page, href):
                        view.click()
                    page.evaluate("window.__readonlyProbe.mark('details.navigation_completed')")
                    ...
                    expect(page.locator('ld-confirm-dropdown').get_by_role('button',name='Confirm',exact=True)).to_be_visible()
                    page.evaluate("window.__readonlyProbe.mark('confirm.observed_visible')")
                    page.wait_for_timeout(450)
                    page.evaluate('window.__readonlyProbe.restoreInjection?.()')
...
if __name__ == '__main__':
    ...
    with os.fdopen(os.open(HERE/'probe.stdout.private.txt',os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600),'wb') as stdout, os.fdopen(os.open(HERE/'probe.stderr.private.txt',os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600),'wb') as stderr:
        ...
        try: p.wait(timeout=55)
            ...
            try: p.wait(timeout=max(.01,60-(time.monotonic()-started)))
