...
from support import validate_project, load_credentials, ProcessLogs, classify, publish_json, inventory
...
parser.add_argument("--case", choices=("healthy", "note-loss", "missing-observation", "expired-session", "lost-response"), default="healthy")
...
publish_json(OUT / "pack-identity.json", inventory(PACK))
...
try:
    ...
    server_args = [str(PACK / 'server.py'), '--project', str(ROOT)]
