...
class StopRequest:
    """Signals cannot unwind Playwright's synchronous dispatcher safely."""
    def __init__(self):
    ...
    def checkpoint(self):
        if self.requested:
            raise ExecutionInterrupted('Stop requested at a native-call boundary')


@contextmanager
def turbo_visit_completed(page, href):
    """Observe the matching promoted visit, not just visible frame contents.

    Pinned linkding promotes its details frame to a history visit. Its later
    before-cache event removes confirmation dropdowns. Arm before the click so
    even a fast completion is observed; a missing event remains a timeout.
    """
    key = '__fa_visit_' + uuid4().hex
    release = """key => {
      const state = globalThis[key];
      if (state) document.removeEventListener('turbo:load', state.listener);
      delete globalThis[key];
    }"""
    page.evaluate("""({key, href}) => {
      if (Object.hasOwn(globalThis, key)) throw new Error('Observer key collision');
      const expected = new URL(href, location.href).href;
      const state = {completed: false};
      state.listener = event => {
        const url = event.detail && event.detail.url;
        if (typeof url === 'string' && new URL(url, location.href).href === expected) {
          state.completed = true;
        }
      };
      globalThis[key] = state;
      document.addEventListener('turbo:load', state.listener);
    }""", {'key': key, 'href': href})
    try:
        yield
        page.wait_for_function('key => globalThis[key]?.completed === true', arg=key)
    except BaseException:
        try:
            page.evaluate(release, key)
        except Exception:
            pass  # Preserve the original operation error if the page was lost.
        raise
    else:
        page.evaluate(release, key)
...
def private_exception(path, error, secrets):
    """Keep diagnostic call sites locally; never emit private exception text."""
