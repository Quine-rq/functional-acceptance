...
try:
    ...
    with page.expect_response(lambda r: urlsplit(r.url).path.rstrip('/') == '/api/bookmarks/check' and r.request.method == 'GET' and parse_qs(urlsplit(r.url).query).get('url') == [URL]) as metadata_response:
        page.get_by_label('URL',exact=True).fill(URL)
