#!/usr/bin/env python3
"""Author-assisted revision, not the independent agent's original result.
Run from checkout: .venv/bin/python .acceptance-study/artifacts/assisted_regression.py
...
"""
import os, sys, json, time, socket, subprocess, hashlib, re, sqlite3
...
OUT = Path(__file__).resolve().parent / ('assisted-run-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ'))
OUT.mkdir()
...
def check(name, passed, **evidence):
    ...
    event('check', name=name, status=checks[name], evidence=evidence)
...
def snap(label):
    ...
    if page.locator('#traceback, #requestinfo').count():
        event('debug_page_evidence_omitted',reason='Avoid settings, request and credential exposure'); return
...
try:
    ...
    check('initial_database_empty', database('initial') == [])
    ...
    check('invalid_url_not_persisted', database('invalid URL') == [])
    snap('invalid-url')
    # Same rendered form, no navigation or reset; correct only the URL.
    with page.expect_response(lambda r: '/api/bookmarks/check/' in r.url and r.request.method == 'GET') as metadata_response:
        page.get_by_label('URL',exact=True).fill(URL)
    metadata = metadata_response.value.json()['metadata']
    check('real_http_metadata', metadata.get('title') == 'Acceptance study article', metadata_title=metadata.get('title'), response_status=metadata_response.value.status)
    ...
    check('delete_target_database', len(remaining) == 1 and remaining[0]['id'] == control_id and remaining[0]['url'] == URL+'&control=1' and remaining[0]['title'] == CONTROL and remaining[0]['notes'] == 'Unrelated control notes' and remaining[0]['owner'] == 'fa_alice' and remaining[0]['shared'] == 0 and set(remaining[0]['tags']) == {'control', RUN}, target_id=target_id, control_id=control_id)
    ...
    check('final_database_empty', database('control cleaned') == [])
    ...
    if unresolved and exit_code == 0: exit_code = 2
