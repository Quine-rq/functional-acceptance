...
class IdentityTests(unittest.TestCase):
    def exercise(self, behavior):
        with tempfile.TemporaryDirectory(prefix='acceptance-identity-') as raw:
            ...
            bootstrap = (
                ...
                'def version(name):\n'
                + ("    raise RuntimeError('synthetic version metadata failure')\n" if behavior == 'version-error'
                   else "    return 'synthetic-1.0'\n")
    ...
    def test_no_plugins_allows_import_and_records_identity(self):
        result, imports, package = self.exercise('healthy')
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(imports, 'application\ncli\n')
        identity = json.loads(result.stdout)
        self.assertEqual(identity['plugins'], [])
        self.assertEqual(identity['package'], str(Path(package) / '__init__.py'))
        self.assertEqual(identity['cli'], str(Path(package) / 'cli.py'))
        self.assertEqual(identity['version'], 'synthetic-1.0')
        self.assertEqual(identity['pytest'], 'synthetic-1.0')

    def test_metadata_failure_stops_before_application_import(self):
        for behavior in ('metadata-error', 'version-error'):
            with self.subTest(behavior=behavior):
                result, imports, _ = self.exercise(behavior)
