...
def journey():
    ...
    probe = "import sys,os,json,sqlite3,sqlite_utils,sqlite_utils.cli,importlib.metadata as m; print(json.dumps({'executable':sys.executable,'python':sys.version,'cwd':os.getcwd(),'uid':os.getuid(),'sqlite':sqlite3.sqlite_version,'package':sqlite_utils.__file__,'cli':sqlite_utils.cli.__file__,'version':m.version('sqlite-utils'),'pytest':m.version('pytest'),'plugins':[e.name for e in m.entry_points(group='sqlite_utils')]}))"
